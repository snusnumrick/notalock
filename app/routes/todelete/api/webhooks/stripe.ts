...
async function handlePaymentIntentSucceeded(
  ...
  try {
    ...
    const paymentResult = await paymentService.verifyPayment(String(paymentIntent.id), 'stripe');
...
async function handleChargeRefunded(
  charge: Record<string, unknown> & { amount_refunded?: number }
) {
  ...
  try {
    ...
    const paymentIntent = (await paymentService.getPaymentIntent(
      String(charge.payment_intent),
      'stripe'
    )) as { metadata?: { orderReference?: string } };
    ...
    if (paymentIntent && paymentIntent.metadata?.orderReference) {
      await updateOrderStatusFromPayment(paymentIntent.metadata.orderReference, {
        ...paymentResult,
        status: 'refunded',
        refundAmount: charge.amount_refunded ? charge.amount_refunded / 100 : 0, // Convert from cents
        refundReason:
