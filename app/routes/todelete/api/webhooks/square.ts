...
async function handlePaymentUpdate(payment: Record<string, unknown>) {
  ...
  try {
    ...
    const paymentResult = await paymentService.verifyPayment(payment.id as string, 'square');
    ...
    if (paymentResult.success) {
      ...
      const orderReference =
        payment.orderId ||
        (payment.metadata && (payment.metadata as Record<string, unknown>).orderReference);
